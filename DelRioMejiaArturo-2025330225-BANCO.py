    ...
    def aplicar():
        try:
            ...
            cur.execute("""INSERT INTO COMPRAS (ClaveRastreo, Saldo, Importe, CCliente, CComercio)
               VALUES (?, ?, ?, ?, ?)""",
               (clave_rastreo, float(saldo), float(monto) if tipo == "deposito" else -float(monto), datos_usuario[0], None))
...
def transferencia(datos_usuario):
    ...
    def transferir():
        try:
            ...
            if monto <= 0 or monto > datos_usuario[-1]:
                raise ValueError("Monto inválido")

            nuevo_saldo_origen = datos_usuario[-1] - monto
            nuevo_saldo_destino = datos[0] + monto
            ...
            cur.execute("""INSERT INTO COMPRAS (ClaveRastreo, Saldo, Importe, CCliente, CComercio)
               VALUES (?, ?, ?, ?, ?)""",
               (clave_rastreo_1, float(nuevo_saldo_origen), -float(monto), datos_usuario[0], None))
            cur.execute("""INSERT INTO COMPRAS (ClaveRastreo, Saldo, Importe, CCliente, CComercio)
               VALUES (?, ?, ?, ?, ?)""",
               (clave_rastreo_2, float(nuevo_saldo_destino), float(monto), destino, None))
...
def registrar_venta(datos_usuario):
    ...
    def guardar():
        try:
            ...
            clave_comercio = int(datos_usuario[0])  # ✅ Convertir a INTEGER
            ...
            with sqlite3.connect("BANCO_EL_BURRO_FELIZ.db") as con:
                ...
                if not resultado:
                    raise ValueError("Cliente no encontrado.")
                saldo_cliente = float(resultado[0])  # ✅ Cast a float

                # Validar existencia del comercio
                cur.execute("SELECT 1 FROM COMERCIO WHERE ClaveComercio=?", (clave_comercio,))
                if not cur.fetchone():
                    raise ValueError("Comercio no encontrado.")

                # ✅ Insertar sin ClaveRastreo (autoincremental)
                cur.execute("""
                        INSERT INTO COMPRAS (Saldo, Importe, CCliente, CComercio)
                        VALUES (?, ?, ?, ?)
                    """, (float(saldo_cliente), float(importe), numero_cliente, clave_comercio))
...
def agregar_cuenta_cliente_tipo(tipo):
    ...
    for campo in campos:
        ...
        if tipo == "Cliente" and campo == "Número CLABE":
            ...
            clave = str(random.randint(0, 9))  # Un solo dígito
            entrada = tk.Entry(ventana)
...
def mostrar_reporte(datos_usuario):
    ...
    try:
        ...
        for venta in ventas:
            ...
            linea = f"Rastreo: {rastreo} | Importe: ${venta[1]:.2f} | Saldo: ${venta[2]:.2f}"
...
def menu_operaciones(tipo_usuario, datos_usuario):
    ...
    if tipo_usuario == "CLIENTE":
        ...
        crear_boton("Procesar", lambda: messagebox.showinfo("Procesar", "Procesando operación...")).pack(pady=5)
